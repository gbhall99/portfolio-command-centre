...
describe('Sprint picker default selection', () => {
  ...
  it('picks the sprint containing today', async () => {
    ...
    const sprints = [
      { sprint_id: 'CY26-S1', customer: 'GCC', start_date: '2026-03-01' },
      { sprint_id: 'CY26-S2', customer: 'GCC', start_date: '2026-04-05' },
      { sprint_id: 'CY26-S3', customer: 'GCC', start_date: '2026-05-10' }
    ];
  ...
  it('falls back to next future sprint when today is between sprints', async () => {
    ...
    const sprints = [
      { sprint_id: 'CY26-S1', customer: 'GCC', start_date: '2026-03-01' },
      { sprint_id: 'CY26-S2', customer: 'GCC', start_date: '2026-06-01' }
    ];
    const app = await loadApp(makeDataset({ projects: [makeProject({ customer: 'GCC' })], sprints, team_members: [makeMember()] }));
    const restore = patchWindowDate(app.window, '2026-05-05');
    try {
  ...
  it('falls back to last past sprint when no future sprint exists', async () => {
    ...
    const sprints = [
      { sprint_id: 'CY26-S1', customer: 'GCC', start_date: '2026-01-01' },
      { sprint_id: 'CY26-S2', customer: 'GCC', start_date: '2026-02-05' }
    ];
    const app = await loadApp(makeDataset({ projects: [makeProject({ customer: 'GCC' })], sprints, team_members: [makeMember()] }));
    const restore = patchWindowDate(app.window, '2026-05-05');
    try {
