...
describe('migrateSchema', () => {
  ...
  it('seeds project.narrative and last_reviewed_at on legacy projects', async () => {
    resetIdSeq();
    const p = makeProject({ id: 'Acme Industries-NAR' });
    const app = await loadApp(makeDataset({ projects: [p] }));
    const proj = app.App.data.projects.find(x => x.id === 'Acme Industries-NAR');
  ...
  it('migrateSchema is idempotent for narrative seeding', async () => {
    resetIdSeq();
    const p = makeProject({ id: 'Acme Industries-NAR2' });
    ...
    const app = await loadApp(makeDataset({ projects: [p] }));
    const proj = app.App.data.projects.find(x => x.id === 'Acme Industries-NAR2');
