...
describe('Solver — R12: concurrent single-person guard', () => {
  it('splits two DE deliveries across different sprints when only one DE member exists (R12 ON)', async () => {
    ...
    const projA = makeProject({ id: 'Acme Industries-A', name: 'Alpha', priority: 1, size_engineering: 5, delivery_config: { phase_order: ['Data Engineering'] } });
    const projB = makeProject({ id: 'Acme Industries-B', name: 'Beta', priority: 2, size_engineering: 5, delivery_config: { phase_order: ['Data Engineering'] } });
  ...
  it('allows two Requirements phases on the same person in the same sprint (allow-list default)', async () => {
    resetIdSeq();
    const sprints = futureSprints(4);
    const projA = makeProject({ id: 'Acme Industries-A', name: 'Alpha', priority: 1, size_engineering: 0, size_requirements: 3, delivery_config: { phase_order: ['Requirements'] } });
    const projB = makeProject({ id: 'Acme Industries-B', name: 'Beta', priority: 2, size_engineering: 0, size_requirements: 3, delivery_config: { phase_order: ['Requirements'] } });
