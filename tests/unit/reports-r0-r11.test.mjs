...
describe('R1 / AC-R1.5 — Reports.Catalogue lists all reports (7 core + costs_report)', () => {
  it('catalogue has 11 entries with required metadata keys', async () => {
    const app = await bootEmpty();
    expect(app.Reports.Catalogue.length).toBe(11);
    ...
    expect(ids).toEqual(['business_case', 'costs_report', 'customer_pack', 'meeting_agenda', 'portfolio_overview', 'portfolio_pack', 'sponsor_pack', 'sprint_brief', 'status_report', 'success_story', 'wireframe_spec'].sort());
    app.teardown();
  });
});
