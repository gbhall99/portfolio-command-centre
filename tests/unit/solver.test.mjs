...
describe('Solver — R8: "Both"-customer capacity split', () => {
  it('calcMemberCapacityForSprint returns capacity_by_customer value for the requested scope', async () => {
    ...
    const bothMember = makeMember({
      ...
      capacity_by_customer: { GCC: 7, KS: 13 }
    });
    const app = await loadApp(makeDataset({
      projects: [], sprints, team_members: [bothMember]
    }));
    // GCC scope sees 7 SP, KS scope sees 13 — NOT the 20 total.
    expect(app.Sprint.calcMemberCapacityForSprint(bothMember, 'CY26-S1', 'GCC').points).toBe(7);
    expect(app.Sprint.calcMemberCapacityForSprint(bothMember, 'CY26-S1', 'KS').points).toBe(13);
    app.teardown();
  });
  ...
  it('back-compat fallback equal-splits when capacity_by_customer is absent', async () => {
    ...
    dataset.customers = [
      { name: 'GCC', color: '#3b82f6', staleThreshold: 14 },
      { name: 'KS',  color: '#10b981', staleThreshold: 14 },
      { name: 'DR&I', color: '#a855f7', staleThreshold: 14 }
    ];
    const app = await loadApp(dataset);
    // With 3 customers and no per-customer map, each scope sees 30/3 = 10.
    const gccCap = app.Sprint.calcMemberCapacityForSprint(bothMember, 'CY26-S1', 'GCC').points;
...
describe('Solver — R11: day-budget clamping', () => {
  it('daysPerSPMultiplier > 1 forces more time-splits than baseline', async () => {
    ...
    const basePlan = baseApp.Solver.solve('GCC', baseApp.Sprint.allocSettings, baseApp.App.data, baseApp.Sprint);
    ...
    const slowPlan = slowApp.Solver.solve('GCC', slowApp.Sprint.allocSettings, slowApp.App.data, slowApp.Sprint);
...
describe('Solver — Determinism', () => {
  it('returns identical allocations on back-to-back runs with identical inputs', async () => {
    ...
    const a = app.Solver.solve('GCC', app.Sprint.allocSettings, app.App.data, app.Sprint);
    const b = app.Solver.solve('GCC', app.Sprint.allocSettings, app.App.data, app.Sprint);
...
describe('Solver — overflow slice integrity (bug fix regression)', () => {
  ...
  it('every slice has points equal to the sum of its assigned_to totals (or is an overflow slice)', async () => {
    ...
    const proj = makeProject({ customer: 'GCC', size_engineering: 15 });
    proj.size_total = 15;
    const member = makeMember({ name: 'A', customer: 'GCC', available_points_per_sprint: 10 });
    const { plan, app } = await runSolver(makeDataset({
