...
describe('migrateSchema — integer points', () => {
  it('rounds fractional story points on projects and splits', async () => {
    ...
    const p = app.App.data.projects.find(x => x.id === 'GCC-DIRTY');
  ...
  it('rounds fractional team member capacity fields', async () => {
    const app = await loadApp(makeDataset({
      ...
      team_members: [{
        ...
        capacity_by_customer: { GCC: 7.5, KS: 6.56 },
    ...
    expect(tm.capacity_by_customer.GCC).toBe(8);
    expect(tm.capacity_by_customer.KS).toBe(7);
