...
describe('calculateProjectPriorityScore', () => {
  it('component formula reconciles with explainPriorityScore total', async () => {
    ...
    const breakdown = app.App.explainPriorityScore('GCC-TEST');
  ...
  it('On-Hold/Complete status scores 0 for status component', async () => {
    resetIdSeq();
    const onHold = makeProject({ id: 'GCC-HOLD', status: 'On Hold' });
    const complete = makeProject({ id: 'GCC-DONE', status: 'Complete' });
    const app = await loadApp(makeDataset({ projects: [onHold, complete] }));
    const holdBreak = app.App.explainPriorityScore('GCC-HOLD');
    const doneBreak = app.App.explainPriorityScore('GCC-DONE');
...
describe('autoPrioritise / applyRecommendedPriority', () => {
  it('autoPrioritise writes recommended_priority but does NOT mutate priority', async () => {
    resetIdSeq();
    const a = makeProject({ id: 'GCC-A', priority: 99, status: 'Blocked', rag_schedule: 'Red', rag_resourcing: 'Red', rag_scope: 'Red' });
    const b = makeProject({ id: 'GCC-B', priority: 1,  status: 'Not Started' });
    const app = await loadApp(makeDataset({ projects: [a, b] }));
    app.App.autoPrioritise();
    const aAfter = app.App.data.projects.find(p => p.id === 'GCC-A');
    const bAfter = app.App.data.projects.find(p => p.id === 'GCC-B');
  ...
  it('applyRecommendedPriority copies recommended → priority and logs audit entry', async () => {
    resetIdSeq();
    const a = makeProject({ id: 'GCC-A', priority: 5, status: 'Blocked' });
    ...
    app.App.applyRecommendedPriority('GCC-A');
  ...
  it('applyAllRecommendations changes only projects where priority !== recommended_priority', async () => {
    resetIdSeq();
    const already = makeProject({ id: 'GCC-MATCH', priority: 2 });
    const diff    = makeProject({ id: 'GCC-DIFF',  priority: 7 });
...
describe('explainPriorityScore', () => {
  it('returns 6 rows in documented order', async () => {
    resetIdSeq();
    const p = makeProject({ id: 'GCC-EXP', status: 'In Progress' });
    const app = await loadApp(makeDataset({ projects: [p] }));
    const b = app.App.explainPriorityScore('GCC-EXP');
