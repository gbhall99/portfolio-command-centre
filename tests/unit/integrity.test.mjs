...
describe('validateDataIntegrity', () => {
  it('flags duplicate project ids', async () => {
    resetIdSeq();
    const a = makeProject({ id: 'Acme Industries-DUP' });
    const b = makeProject({ id: 'Acme Industries-DUP' });
    const app = await loadApp(makeDataset({ projects: [a, b] }));
    const issues = app.App.validateDataIntegrity();
    expect(issues.some(i => i.message.includes('Duplicate project ID: Acme Industries-DUP'))).toBe(true);
    app.teardown();
  });

  it('flags dependencies pointing to non-existent projects', async () => {
    resetIdSeq();
    const p = makeProject({ id: 'Acme Industries-P', dependencies: [{ type: 'blocked_by', target_id: 'NOT-REAL' }] });
