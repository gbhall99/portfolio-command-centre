...
describe('calcMemberCapacityForSprint', () => {
  it('honours sprint_overrides.available_points', async () => {
    ...
    expect(app.Sprint.calcMemberCapacityForSprint(member, 'CY26-S1', 'Acme Industries').points).toBe(5);
    expect(app.Sprint.calcMemberCapacityForSprint(member, 'CY26-S2', 'Acme Industries').points).toBe(20);
    app.teardown();
  });
  ...
  it('linear ramp profile zeros capacity before start_date', async () => {
    ...
    const mc = app.Sprint.calcMemberCapacityForSprint(member, 'CY26-S1', 'Acme Industries');
...
describe('calcSkillCapacityForSprint', () => {
  it('sums members\' capacity for the requested skill and customer', async () => {
    const team = [
      makeMember({ name: 'Alice', customer: 'Acme Industries', available_points_per_sprint: 15, primary_skills: ['Data Engineering'] }),
      makeMember({ name: 'Bob',   customer: 'Acme Industries', available_points_per_sprint: 10, primary_skills: ['Data Engineering'] }),
      makeMember({ name: 'Carol', customer: 'Globex',  available_points_per_sprint: 20, primary_skills: ['Data Engineering'] })  // not Acme Industries
    ];
    const app = await loadApp(makeDataset({ team_members: team, sprints: makeSprintSequence(1) }));
    const cap = app.Sprint.calcSkillCapacityForSprint('Acme Industries', 'CY26-S1');
    // Alice + Bob = 25. Carol excluded because customer !== Acme Industries.
