...
describe('Settings IA registry', () => {
  ...
  it('expected category ids are present', async () => {
    ...
    expect(ids.sort()).toEqual(['customers','data','display','metrics','objectives','personas','scheduler','scoring','sprints','team','templates']);
    app.teardown();
  });
});
...
describe('Dashboard rendering', () => {
  it('renders 11 tile buttons by default', async () => {
    const app = await loadApp(makeDataset({ projects: [makeProject()] }));
    app.App.activeCustomer = 'Acme Industries';
