...
test('Dates section in Delivery has only hard_deadline + target_date editors', async ({ page }) => {
  await openAppWithData(page);
  const id = await page.evaluate(() => (window as any).App.data.projects[0].id);
  await page.evaluate((pid) => (window as any).DetailPanel.open(pid), id);
  await expect(page.locator('#detailPanel.open')).toBeVisible();
  // The Dates panel-section title should appear exactly once
  await expect(page.locator('#detailPanel .panel-section-title', { hasText: /^Dates$/ })).toHaveCount(1);
  ...
  // hard_deadline + target_date inputs ARE present
