...
```
1. User clicks "Set logo…" on the GCC row in Customers
2. Inline editor opens (textarea + Save/Remove)
3. User pastes a URL or data-URL → Save
4. App.setCustomerLogo('GCC', value) → App.setBranding('GCC', { logo: value })
5. data.customers.find(GCC).logo = value (existing write path)
...
- `App.setCustomerLogo('GCC', 'https://example/logo.png')` writes to `data.customers.find(GCC).logo`.
...
- Click "Set logo…" on the GCC row → enter a URL → Save → assert `<img>` appears with that src.
