...
| 3     | Customer      | Identity      | yes (select: Acme Industries/Globex/Initech) |
...
```
1. User double-clicks Target cell on row Acme Industries-001
...
5. Equal to original? no → App.updateProject('Acme Industries-001', 'target_date', '2026-07-15')
...
```
1. User edits "DE points" cell on row Acme Industries-001 from 8 → 12
2. App.updateProject('Acme Industries-001', 'size_engineering', 12)  [audit: size_engineering 8→12]
3. onAfterWrite hook computes new total = 5+13+12+0+3 = 33
4. p.size_total was 29 → App.updateProject('Acme Industries-001', 'size_total', 33)  [audit: size_total 29→33]
...
| Customer changed inline (e.g. Acme Industries → Globex)    | write succeeds; project disappears from current view (customer-scoped). User sees toast `Moved to Globex` with Undo affordance via existing global undo. |
...
4. **Inline edit a date**: double-click Target on Acme Industries-001 → type `2026-07-15` → Tab → assert cell shows `15 Jul` AND `App.data.projects[0].target_date === '2026-07-15'` AND localStorage `appData` reflects it.
