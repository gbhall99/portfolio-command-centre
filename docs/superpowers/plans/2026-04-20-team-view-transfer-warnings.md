...
describe('Sprint._validateSliceMove', () => {
  it('hard-fails when destination sprint has zero skill capacity for this skill', async () => {
    ...
    const res = app.Sprint._validateSliceMove({ projectId: 'Acme Industries-A', skillKey: 'size_tableau', fromSprint: 'CY26-S1', toSprint: 'CY26-S2', points: 5 });
...
```javascript
  it('emits phase-order warning when moving a later-phase slice earlier than an earlier phase', async () => {
    ...
    const res = app.Sprint._validateSliceMove({ projectId: 'Acme Industries-PH', skillKey: 'size_uat_adoption', fromSprint: 'CY26-S3', toSprint: 'CY26-S1', points: 5 });
...
```javascript
  it('hard-fails when destination sprint is after the project hard deadline', async () => {
    ...
    const res = app.Sprint._validateSliceMove({ projectId: 'Acme Industries-DL', skillKey: 'size_engineering', fromSprint: 'CY26-S1', toSprint: 'CY26-S3', points: 5 });
    expect(res.hardFail).toBe(true);
