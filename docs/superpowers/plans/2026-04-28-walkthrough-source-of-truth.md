...
2. `recordWalkthroughAction` with `owner === 'Jordan'` matching a `team_members[].name` → action gains `personal_owner_id === 'Jordan'`.
...
Create `tests/unit/pack-data.test.mjs` with four cases for `computeCustomerPackData('Acme Industries')`:
...
Append two cases:
1. `Report.buildProjectPackDoc('Acme Industries-X')` returned HTML contains the project's `narrative.headline` text.
...
1. After `Walkthrough.open('Acme Industries')`, the overlay exists and contains the class names `wt-top`, `wt-list`, `wt-center`, `wt-cust`, `wt-bottom`.
...
2. Calls `Walkthrough.open('Acme Industries')` via `page.evaluate`.
