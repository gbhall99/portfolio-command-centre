...
describe('Sprint picker default selection', () => {
  ...
  it('picks the sprint containing today', async () => {
    const sprints = [
      { sprint_id: 'CY26-S1', customer: 'GCC', start_date: '2026-04-01', end_date: '2026-04-28' },
      { sprint_id: 'CY26-S2', customer: 'GCC', start_date: '2026-04-29', end_date: '2026-05-26' },
      { sprint_id: 'CY26-S3', customer: 'GCC', start_date: '2026-05-27', end_date: '2026-06-23' }
    ];
    const restore = fixedDate('2026-05-05');
    try {
      const app = await loadApp(makeDataset({ projects: [makeProject({ customer: 'GCC' })], sprints, team_members: [makeMember()] }));
  ...
  it('falls back to next future sprint when today is between sprints', async () => {
    const sprints = [
      { sprint_id: 'CY26-S1', customer: 'GCC', start_date: '2026-04-01', end_date: '2026-04-15' },
      { sprint_id: 'CY26-S2', customer: 'GCC', start_date: '2026-05-10', end_date: '2026-05-30' }
    ];
    const restore = fixedDate('2026-05-05');
    try {
      const app = await loadApp(makeDataset({ projects: [makeProject({ customer: 'GCC' })], sprints, team_members: [makeMember()] }));
  ...
  it('falls back to last past sprint when no future sprint exists', async () => {
    const sprints = [
      { sprint_id: 'CY26-S1', customer: 'GCC', start_date: '2026-01-01', end_date: '2026-01-28' },
      { sprint_id: 'CY26-S2', customer: 'GCC', start_date: '2026-02-01', end_date: '2026-02-28' }
    ];
    const restore = fixedDate('2026-05-05');
    try {
      const app = await loadApp(makeDataset({ projects: [makeProject({ customer: 'GCC' })], sprints, team_members: [makeMember()] }));
