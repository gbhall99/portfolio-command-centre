...
- **Assistant**: per-customer in-memory threads (chat history never enters `App.data`; opt-in localStorage persistence via the Remember toggle — message text only, proposals never persisted); citations deep-link entities; proposals render diff cards with Confirm/Discard.
