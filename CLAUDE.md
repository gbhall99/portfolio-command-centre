...
- **Billing — two sides**: COST = `settings.rate_card[skill]={perm,contract}` (per-SP internal cost, feeds `App.computeProjectCost` BAC/EV/AC). SELL = `settings.billing` (`currency`, `sell_rates[skill]` per-SP) + `data.billing_arrangements[]` (customer-scoped prepaid blocks: `skill|'any'`, `prepaid_points`, `amount_invoiced`). `Billing.customerSummary` is the single source of truth: completed work (EV semantics) draws prepaid pools down first (deterministic: projects by id, skill-specific pools before 'any'), remainder bills as T&M. Quotes (`Billing.quoteForProject`) price PLANNED points netting only the REMAINING prepaid balance. The 'quoted' SOW template set requires a generated quote before approval (`validation.requires_quote`); `Sow.setQuote` stores it and renders Commercials — figures are never hand-written.
