...
- **Customer-scoped** — every view requires a customer selection (Acme Industries, Globex, Initech). No "All" option exists. Capacity calculations filter team members by customer (including those marked "Both"). Auto-selects first customer on data load.

## Data Model Key Facts
- **Customer** is always single-select and mandatory: Acme Industries, Globex, Initech. Never allow an "All" option.
...
- **Dependencies**: stored as strings in data (e.g. "Acme Industries-001"), not arrays. Use `Array.isArray()` checks when iterating.
- **Team Members**: each has a `customer` field (Acme Industries, Globex, or "Both"), `primary_skills`, `available_points_per_sprint`. Capacity is split equally across primary skills.
