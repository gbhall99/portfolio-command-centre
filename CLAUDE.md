...
- **Billing — two sides**: COST = `settings.rate_card[skill]={perm,contract}` (per-SP internal cost, feeds `App.computeProjectCost` BAC/EV/AC). SELL = `settings.billing`: **hourly rates by region × level** (`rate_table[region][level]`), `hours_per_point` conversion (default 8), `customer_defaults[customer]={region,level}` (quote basis + unassigned work), plus `data.billing_arrangements[]` (customer-scoped prepaid blocks: `skill|'any'`, `prepaid_points`, `amount_invoiced`). Team members carry optional `region`/`level` — completed work bills at the doer's band, blended per skill. `Billing.customerSummary` is the single source of truth: completed work (EV semantics) draws prepaid pools down first (deterministic: projects by id, skill-specific pools before 'any'), remainder bills as T&M hours. Quotes (`Billing.quoteForProject`) price PLANNED points as hours at the customer's default band, netting only the REMAINING prepaid balance. The 'quoted' SOW template set requires a generated quote before approval (`validation.requires_quote`); `Sow.setQuote` stores it and renders Commercials — figures are never hand-written.
